...
export default function EndCustomerCreationForm() {
  ...
  // Basic Tailwind-styled input component (can be moved to a shared components folder)
  const InputField = ({ name, label, type = "text", registerFn, errorMsg, ...props }: any) => (
    <div className="mb-4">
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white p-6 md:p-8 rounded-lg shadow">
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-1">Billing Visibility</label>
        <div className="flex items-center">
          ...
          />
          <label htmlFor="billingVisibility" className="ml-2 block text-sm text-gray-900">
            Allow client to view billing for this end customer
          </label>
        </div>
        {errors.billingVisibility?.message && <p className="mt-1 text-xs text-red-600">{errors.billingVisibility.message}</p>}
      </div>
      ...
      <div className="pt-5">
        ...
        >
          {isPending || isSubmitting ? 'Creating End Customer...' : 'Create End Customer'}
        </button>
      </div>
    </form>
  );
